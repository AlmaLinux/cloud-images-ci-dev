...
for rid in "${RUN_IDS_IMPORT[@]}"; do
  ...
  # Backticks below are literal markdown code-fence characters around the
  # OCID body in the rendered summary line; not command substitution.
  # shellcheck disable=SC2016
  ocid=$(job_log "${job_id}" \
    | grep -oE '^\*\*Compute Image OCID\*\*: `ocid1\.image\.oc1\.[^`]+`' \
    | head -1 \
    | sed -E 's/^\*\*Compute Image OCID\*\*: `(.+)`$/\1/')
...
for rid in "${RUN_IDS_TEST[@]}"; do
  ...
  block=$(job_log "${job_id}" \
    | grep -E '^\*\*(Compute Custom Image|Test Instance|Shape|AlmaLinux release|System architecture|Test)\*\*:' \
    | head -10)
